...
| `forms.html` | 常用表單下載（不用午餐須先行離校申請單、請假單、臨時外出單、手機暨行動載具同意書） |
...
- **forms.html**：表單下載按鈕直接連到**原網站 assets 的 Word（.doc）／PDF 檔**（`piliboychi.github.io/guandu-uniform/assets/…`），所以**請保留舊網站、不要刪除 assets 資料夾**，家長才能正常下載。（注意：原網站的「WORD 檔」按鈕其實是壞的——檔案為 .doc 而非 .docx，本版已修正。）🆕 已新增「**不用午餐須先行離校申請單**」（因應生生吃午餐、中午放學延後至 12:40，12:00 提早離校須申請），PDF 檔請上傳至 `assets/` 資料夾。
- **traffic.html**：上放學交通與接送宣導。含家長會與志工團製作之「上下課安全提醒」海報（圖片放在 **`assets/safety-poster.jpg`**，上傳時要放進 `assets` 資料夾，不要放根目錄）；放學時間為**中午 12:40、下午 15:50**，並說明中午 12:00 未經申請不得提早離校、可下載「不用午餐須先行離校申請單」。
...
4. 再上傳海報圖片：點進專案裡的 **`assets` 資料夾** → 「Add file → Upload files」→ 上傳 **`safety-poster.jpg`**。這張圖**必須放在 `assets` 資料夾內**，`traffic.html` 才能讀到（網址為 `…/guandu-uniform/assets/safety-poster.jpg`）。同時把 **`不用午餐須先行離校申請單.pdf`** 一起上傳進 `assets` 資料夾（網址為 `…/guandu-uniform/assets/不用午餐須先行離校申請單.pdf`），`forms.html` 的下載按鈕才能用。
